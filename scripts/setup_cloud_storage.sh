...
BUCKET_NAME="${GCS_BUCKET_NAME:-scriptsense-exams}"
...
cat > /tmp/scriptsense_cors.json <<'EOF'
[{
...
EOF
gsutil cors set /tmp/scriptsense_cors.json "gs://$BUCKET_NAME"

# Lifecycle: auto-delete raw PDFs after 90 days to save costs
cat > /tmp/scriptsense_lifecycle.json <<'EOF'
{
  "rule": [{
    "action": {"type": "Delete"},
...
EOF
gsutil lifecycle set /tmp/scriptsense_lifecycle.json "gs://$BUCKET_NAME"
